...
function Notification() {
  ...
  const getNotification = () => {
    ...
    let afterDefaultURL = "";
    if (chooseOption) {
      afterDefaultURL = `notifications?for-admin=true&order-by=DateCreate&order-type=DESC&page-offset=1&limit=${limit}`;
    } else {
      afterDefaultURL = `notifications?for-admin=true&is-seen=false&order-by=DateCreate&order-type=DESC&page-offset=1&limit=${limit}`;
    }
